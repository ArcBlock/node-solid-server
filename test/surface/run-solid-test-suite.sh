...
docker build -t web-access-control https://github.com/solid/test-suite.git#master:/testers/web-access-control
...
docker ps -a
docker logs server
export COOKIE="`docker run --network=testnet cookie`"
...
docker run --rm --network=testnet --env COOKIE="$COOKIE" --env-file /tmp/env-vars-for-test-image.list web-access-control
